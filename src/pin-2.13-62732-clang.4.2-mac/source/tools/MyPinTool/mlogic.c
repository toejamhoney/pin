...
#define P_NUM 4
int P_LEN[P_NUM] = {3,3,4,3};
int P_WND[P_NUM] = {3,3,3,3};
...
#define REPORT() if(t->step == t->goal) { printf("2: Pattern is satisfied in [ %i, %i ]\t", CLOCK - window_size, CLOCK+1); printf(" data@%i:", CLOCK - window_size+1);int e;for(e=0;e<window_size+1;e++){printf(" %i",data[CLOCK-window_size+e]);}printf( "\n" );}
#define PUSH(K) if(PATTERNS[K]){ get_tail(PATTERNS[K])->next = new_clause(K); }else{ PATTERNS[K]=new_clause(K); }
#define TRANS(K, S) if(PATTERNS[K]){ t=PATTERNS[K]; window_size = P_WND[K]; while(t){ if(t->step==S){ t->step++; } REPORT() t=t->next; } }
...
int fast_push_code ( int code, int * data)
{
    struct cnode * t;
    int window_size = 0;
    int k = 0;
...
int stream_query( int * data, int data_size ,int test ){
    ...
    {
        init_cnode_array();    
        for ( CLOCK = 0; CLOCK < data_size ; CLOCK ++ ){
            cur = data[CLOCK] ;
            fast_push_code(cur, data);
        }
        free(PATTERNS);
    }
}
...
int main(){
    ...
    int data2[10] = { 1,1,2,3,3,5,1,7,5,3 };
    int k = 0;
    int LIM = 1000;
