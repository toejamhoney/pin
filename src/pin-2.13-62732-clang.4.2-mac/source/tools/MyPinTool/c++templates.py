...
VOID pre_%s(CHAR * name, %s) 
{
    ...
    /* BASELINE */
    TraceFile << "PRE CALL-OUT (PID:"<< PID << ",TID:" << TID << "): " << name << " ("<< %s <<") " << dec << endl;
...
/* For Callout Sequences */
std::string callout_str;
int callout_seq[TICUB] = { };
u_int64_t callout_cnt[TICUB] = { };
int tic = 0;

bool RE_match(std::string re)
{
    return std::regex_match(callout_str, std::regex(re));
}
