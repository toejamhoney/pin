...
VOID pre_%s(CHAR * name, %s) 
{
    ...
    /* BASELINE */
    TraceFile << "IN (PID:"<< PID << ",TID:" << TID << "): " << name << " ("<< %s <<") " << dec << endl;
...
VOID post_%s(CHAR * name, ADDRINT ret)
{
     ...
     * BASELINE
     */
    TraceFile << "CALL-OUT (PID:"<<PID<<",TID:"<<TID<<") OUT: "<< name << " RETURN-VALUE @:" << hex << ret << dec << " RETURN-VALUE-INTERP:" << dec << ((int) ret ) << endl; 
